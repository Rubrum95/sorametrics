...
const TRANSLATIONS = {
    es: {
        ...
        // Nuevas claves
        bridge_contract: "Contrato Bridge", bridge_internal_hash: "Este hash es un ID interno del bridge de SORA. No es visible en Etherscan.", view: "Más Info", direction: "Dirección", view_on_subscan: "Ver en Subscan", transaction_hash: "Hash de Transacción", ethereum_request_hash: "Hash de Solicitud Ethereum", origin: "Origen", extrinsic_id: "ID de Extrinsic", ethereum_network: "Red Ethereum", no_external_link: "Sin enlace externo disponible",
    ...
    jp: {
        ...
        bridge_contract: "ブリッジ契約", bridge_internal_hash: "このハッシュはSORAブリッジの内部IDです。Etherscanでは見えません。", view: "詳細", direction: "方向", view_on_subscan: "Subscanで見る", transaction_hash: "トランザクションハッシュ", ethereum_request_hash: "Ethereumリクエストハッシュ", origin: "オリジン", extrinsic_id: "Extrinsic ID", ethereum_network: "イーサリアムネットワーク", no_external_link: "外部リンクなし",
    ...
    pt: {
        ...
        bridge_contract: "Contrato Bridge", bridge_internal_hash: "Este hash é um ID interno da bridge SORA. Não é visível no Etherscan.", view: "Mais Info", direction: "Direção", view_on_subscan: "Ver no Subscan", transaction_hash: "Hash da Transação", ethereum_request_hash: "Hash de Solicitação Ethereum", origin: "Origem", extrinsic_id: "ID do Extrinsic", ethereum_network: "Rede Ethereum", no_external_link: "Sem link externo disponível",
    ...
    it: {
        ...
        bridge_contract: "Contratto Bridge", bridge_internal_hash: "Questo hash è un ID interno del bridge SORA. Non è visibile su Etherscan.", view: "Più Info", direction: "Direzione", view_on_subscan: "Vedi su Subscan", transaction_hash: "Hash della Transazione", ethereum_request_hash: "Hash Richiesta Ethereum", origin: "Origine", extrinsic_id: "ID Extrinsic", ethereum_network: "Rete Ethereum", no_external_link: "Nessun link esterno disponibile",
...
function openTab(name) {
    ...
    if (bar) {
        if (searchTabs.includes(name)) {
            ...
            if (input) input.placeholder = lang.search_placeholder || 'Search wallet, tx hash, block, extrinsic ID...';
        } else {
        ...
        if (tab === 'xor' && supplyRes.xorMarket) {
            ...
            supplyCardHtml = `
                ...
                <div class="burn-stat-card">
                    ...
                    <div class="burn-stat-sub">${burned30d > 0 ? '-' + formatBurnNumber(Math.abs(burned30d)) + ' ' + esc(lang.burn_pkg || 'pkg.') : esc(lang.burn_no_change || 'No change detected')}</div>
                </div>
                <div class="burn-stat-card">
                    ...
                    <div class="burn-stat-sub">${burnedAll > 0 ? '-' + formatBurnNumber(Math.abs(burnedAll)) + ' ' + esc(lang.burn_pkg || 'pkg.') + ' · $' + formatBurnNumber(burnedAllUsd) : esc(lang.burn_no_change || 'No change detected')}</div>
                </div>
            `;
        } else {
...
function renderBurnFlowSvg(container, data) {
    ...
    if (feesUnpacked) {
        svg += `<text x="55" y="270" text-anchor="middle" fill="${sub}" font-size="6.5" opacity="0.6">${formatBurnNumber(fees)} ${esc(lang.burn_pkg || 'pkg.')}</text>`;
    }
    svg += `</g>`;
    ...
    try {
        ...
        data.forEach(d => {
            ...
            row.innerHTML = `
                ...
                <td>${resultIcon}</td>
                <td>
                    <button class="btn-ghost" onclick="openExtrinsicDetail('${esc(d.extrinsic_id)}')" style="font-size:11px; padding:2px 6px;">&#128269; ${TRANSLATIONS[currentLang]?.view || 'More Info'}</button>
                </td>
            `;
            tbody.appendChild(row);
        });
        ...
        data.forEach(d => {
            ...
            tbody.innerHTML += `
                <tr>
                    ...
                    <td>
                        <button class="btn-ghost" onclick="openTxModal('${esc(d.hash)}', '${esc(d.extrinsic_id)}', '${esc(d.usd_value || '')}')" style="font-size:11px; padding:2px 6px;">🔍 ${TRANSLATIONS[currentLang]?.view || 'More Info'}</button>
                    </td>
                </tr>
            `;
        });
